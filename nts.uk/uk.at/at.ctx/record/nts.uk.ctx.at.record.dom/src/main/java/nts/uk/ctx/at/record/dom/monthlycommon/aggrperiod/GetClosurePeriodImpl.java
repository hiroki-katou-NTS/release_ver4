...
public class GetClosurePeriodImpl implements GetClosurePeriod {
	...
	public List<ClosurePeriod> fromYearMonth(String employeeId, GeneralDate criteriaDate, YearMonth yearMonth,
		...
		ClosurePeriodCacheKey cachekey = new ClosurePeriodCacheKey(employeeId, criteriaDate, Optional.of(yearMonth), Optional.empty());
		if (cachedClosurePeriod.containsKey(cachekey)) {
			return cachedClosurePeriod.get(cachekey);
		}
		...
		while (itrClosurePeriods.hasNext()) {
			...
			while (itrAggrPeriods.hasNext()) {
				...
				if (!aggrPeriod.getClosureMonth().yearMonth().equals(yearMonth)) itrAggrPeriods.remove();
			}
			if (target.getAggrPeriods().size() == 0) itrClosurePeriods.remove();
		}

		cachedClosurePeriod.put(cachekey, closurePeriods);
	...
	public List<ClosurePeriod> fromPeriod(String employeeId, GeneralDate criteriaDate, DatePeriod period,
		...
		ClosurePeriodCacheKey cachekey = new ClosurePeriodCacheKey(employeeId, criteriaDate, Optional.empty(), Optional.of(period));
		if (cachedClosurePeriod.containsKey(cachekey)) {
			return cachedClosurePeriod.get(cachekey);
		}
		...
		List<ClosurePeriod> closurePeriods = this.calcPeriodForAggregate.fromClosureHistory(employeeId, criteriaDate, closureIdHistoryList);
		cachedClosurePeriod.put(cachekey, closurePeriods);
